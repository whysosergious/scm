...
- JSON API under `/api`: config get/put; projects list/import/delete; `{id}/checkout`, `{id}/ensure-content-dir`, `{id}/content` (+`/{name}` get/put), `{id}/git/status`, `{id}/media` (list/serve/upload/rename/delete), `{id}/publish`. Errors use `{"error":{category,message,detail}}`; publish returns HTTP 200 with a discriminated `outcome` field covering spec §11 cases.
- Publishing stages the configured content dir plus media_dir (when present); local content is never reverted on failure.
