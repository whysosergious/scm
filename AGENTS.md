...
Current state: v1 backend and frontend are implemented. Backend modules: `error.rs` (`ScmError`, category + status → JSON error shape), `paths.rs` (path-safety helpers, unit-tested), `config.rs` (load/validate/atomic-save `scm-config.json`, unknown keys preserved via `#[serde(flatten)]`), `git.rs` (git CLI wrapper over `tokio::process`), `project.rs` (clone-on-demand checkout lifecycle + verification), `content.rs` (direct-children `.json` discovery/load/save), `setup.rs` (`AppState`: `Arc<RwLock<AppConfig>>` — no actors). API lives in `src/http/api.rs`; frontend is decomposed under `web/{styles,scripts}` with `scripts/main.js` booting a small pub/sub store (`state.js`) + fetch layer (`api.js`). The file editor (`components/json-editor.js`) is tabbed **Form | JSON** per `spec.md` §9: `json-model.js` is an order-preserving tree model (own JSON parser — `JSON.parse` reorders integer-like keys; serialization emits from the tree for the same reason), `components/form-editor.js` renders recursive property forms (title-inputs, type dropdown, resizable textarea for strings, toggle/number value inputs, index badges for array entries, per-row clone/collapse/delete cluster on the right, "(n items)" counts), `components/dnd.js` does cross-parent pointer drag & drop with FLIP animations and a cycle guard. Media management (`components/media.js`): sidebar Media button, four view modes (small grid / large grid / list-medium / list-small), upload, per-item actions (copy link / rename / delete), lightbox viewer with arrows + keyboard + swipe navigation. Legacy `/assets` endpoint superseded by media endpoints. The legacy `/project/*` oxc-transpile route and its deps were removed (owner-approved). Page editor: spec §19 is the authoritative spec. Canvas uses a same-origin `<iframe>` for document isolation; the iframe has `pointer-events:none` and all interaction is handled through a transparent overlay using coordinate-mapped hit-testing (`canvas-iframe.js`). Drag-and-drop uses pointer events (not the HTML DnD API). The transform frame (resize handles) is rendered in the parent document. SVG nodes store innerHTML via XMLSerializer; SVG is not selectable via elementFromPoint (unsolved). HTML import uses DOMParser with full fidelity: preserves all attributes, original tag names, SVG innerHTML, scripts, head elements. `page-html-import.js` includes `mergeWhitespaceNodes()` for whitespace normalization. CodeMirror 6 is bundled in `web/codemirror-src/` (vite, library mode); the `<code-editor>` custom element is used in the SVG inspector and HTML import modal. Frontend dependencies: ProseMirror (rich-editor), CodeMirror 6 (code-editor), both lazy-loaded.
...
- JSON API under `/api`: config get/put; projects list/import/delete; `{id}/checkout`, `{id}/ensure-content-dir`, `{id}/content` (+`/{name}` get/put), `{id}/git/status`, `{id}/media` (list/serve/upload/rename/delete), `{id}/pages` (+`/{name}` get/put/delete, `/{name}/generate`, `/import`, `/{name}/preview`), `{id}/publish`. Errors use `{"error":{category,message,detail}}`; publish returns HTTP 200 with a discriminated `outcome` field covering spec §11 cases.
- Publishing stages the configured content dir plus media_dir (when present) plus pages/ (when present); local content is never reverted on failure.
...
- Page editor nesting rules: a div cannot be dropped inside an h1 or p; a span can be inside a p or h1; drop targets that violate the parent→child compatibility matrix (spec.md §19.7) are rejected. The visual editor canvas must show valid/invalid drop indicators. Both page JSON source files and generated HTML are committed to the target repository.

## Repo quirks

- `.gitignore` ignores `*.lock`, so `Cargo.lock` is untracked — don't commit it.
- Rich text editor: source + npm project in `web/editor-src/` (ProseMirror incl. prosemirror-markdown, prosemirror-inputrules, vite). Build once with `cd web/editor-src && npm install && npm run build`; output `web/scripts/vendor/rich-editor.bundle.js` is committed and lazy-imported at runtime — never run npm/vite to launch the frontend. String values autodetect their format (HTML vs Markdown vs plain) and the rich editor round-trips whichever format it detected; mode state is keyed by property path, not node id.
- CodeMirror 6: source + npm project in `web/codemirror-src/` (vite, library mode). Build once with `cd web/codemirror-src && npm install && npm run build`; output `web/scripts/vendor/code-editor.bundle.js` is committed and lazy-imported at runtime. The `<code-editor>` custom element exposes `value`, `language`, `theme`, `tab-size`, `line-numbers`, `word-wrap`, `font-size` attributes. Used in the SVG inspector and HTML import modal.
